...
venn.write(str(len(uniquegenomeA)) + ',' + str(len(genomeB))+ ',' + str(len(reciprocalAb))+ ',' + str(len(genomeC))+ ',' + str(len(reciprocalAc))
           + ',' + str(len(reciprocalBc))+ ',' + str(len(matches)) + '\n')
...
plt.title(test[0])
plt.savefig('gene_counts_ecoli' + '.pdf', dpi = 600, format = 'pdf', bbox_inches = 'tight')
plt.close()
venn.close()
